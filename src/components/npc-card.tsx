...
export class NpcCard extends Component<any, any> {
	...
	getHeight(min: number, max: number, skew: number): string {
	...
	generateNpc = () => {
		...
		Promise.all([npcRace, npcClass, npcLevel, npcAlignment]).then((r) => {
			...
			const gender = this.getNpcGender();
				...
				<div className="npc-card" >
					...
					<div className="npc-card__bio">
						<ol className="npc-card__attributes">
							...
							<li><strong>Height</strong>{this.getHeight(races.get(npc.race.index)!.height.min, races.get(npc.race.index)!.height.max, 1)}</li>
